...
import { JobToBeDone, Competitor } from '@/data/jobsToBeDone';
...
interface TechnologyDrillDownProps {
  technology: string;
  jobs: JobToBeDone[];
  onClose: () => void;
}

export const TechnologyDrillDown = ({ technology, jobs, onClose }: TechnologyDrillDownProps) => {
  ...
  // Get all competitors using this technology
  ...
  // Get all industries using this technology
  const allIndustries = [...new Set(jobs.map(job => job.industry))];
  ...
  const allRelatedTechnologies = [...new Set(jobs.flatMap(job => job.technologyRequirements))];

  // Technology maturity assessment
  const getTechnologyMaturity = () => {
    const highCompetition = jobs.filter(job => job.competitionLevel === 'High').length;
    const totalJobs = jobs.length;
    const maturityRatio = highCompetition / totalJobs;
    
    if (maturityRatio > 0.6) return { level: 'Mature', description: 'Well-established technology with high competition' };
    if (maturityRatio > 0.3) return { level: 'Growing', description: 'Technology gaining adoption with moderate competition' };
    return { level: 'Emerging', description: 'New technology with low competition and high potential' };
  };

  const maturity = getTechnologyMaturity();
      ...
      <div className="max-w-7xl w-full max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-apple">
        <div className="p-8">
          ...
          <div className="flex items-start justify-between mb-8">
            <div className="space-y-2">
              ...
              <p className="text-lg text-muted-foreground">
                Technology Analysis & Market Opportunities
              </p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            ...
            <Card className="p-6 bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{allIndustries.length}</p>
                  <p className="text-sm text-purple-700">Industries</p>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="mb-8">
            ...
            <Card className="p-6 border">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-semibold text-foreground">{maturity.level}</h3>
                  <p className="text-muted-foreground">{maturity.description}</p>
                </div>
                <Badge 
                  variant="outline" 
                  className={
                    maturity.level === 'Mature' ? 'text-red-600 border-red-300' :
                    maturity.level === 'Growing' ? 'text-orange-600 border-orange-300' :
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {allRelatedTechnologies.filter(tech => tech !== technology).map((tech, index) => (
                <Badge 
                  key={index} 
                  variant="secondary" 
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-4 flex items-center gap-2">
              ...
              Key Competitors Using {technology}
            </h2>
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-4 flex items-center gap-2">
              ...
              Market Opportunities Using {technology}
            </h2>
          ...
          {selectedCompetitor && (
            <div className="space-y-6">
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                <h4 className="font-semibold text-blue-800 mb-2">Technology Strategy</h4>
                <p className="text-sm text-blue-700">
                  {selectedCompetitor.marketShare.includes('45') || selectedCompetitor.marketShare.includes('35') 
                    ? 'Technology leader with significant competitive advantages. Focus on differentiation and innovation to compete effectively.'
                    : selectedCompetitor.marketShare.includes('25') || selectedCompetitor.marketShare.includes('20')
                    ? 'Established player with strong technology position. Consider partnerships or acquisition opportunities.'
                    : 'Emerging competitor with growth potential. Monitor their technology development and market expansion strategies.'
                  }
                </p>
              </div>
            </div>
